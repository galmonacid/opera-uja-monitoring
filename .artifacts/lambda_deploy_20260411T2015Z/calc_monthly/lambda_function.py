...
def handler(event, context):
    ...
    if not target_month:
        target_month = last_closed_month_key()
...
def last_closed_month_key(now=None):
    local_now = ensure_utc_datetime(now).astimezone(LOCAL_TIMEZONE)
    first_of_current_month = datetime(local_now.year, local_now.month, 1, tzinfo=LOCAL_TIMEZONE)
